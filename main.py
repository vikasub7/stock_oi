...
from bokeh.models import AutocompleteInput,ColumnDataSource, Span, SingleIntervalTicker, LinearAxis, PreText, Div, Button, LabelSet, Label, DaysTicker, Range1d, DataTable, TableColumn
...
def plot_des():
    ...
    pv1 = p.vbar(x=dodge('strikePrice', -width/2, range=p.x_range), top='CE_OI_Change', source=source, color="#E74C3C", width=width,
           fill_alpha=0.5)
    pv2 = p.vbar(x=dodge('strikePrice', width/2, range=p.x_range), top='PE_OI_Change', source=source, color="#1E90FF", width=width,
           fill_alpha=0.5)
    ...
    p2 = figure(name='oi', x_range=(minRange, maxRange), plot_width=650, plot_height=350, x_minor_ticks=2,
                ...
                toolbar_location="above")
    p2v1 = p2.vbar(x=dodge('strikePrice', -width/2, range=p2.x_range), top='CE_OI', source=source, color="#E74C3C", width=width,
            fill_alpha=0.5)
    p2v2 = p2.vbar(x=dodge('strikePrice', width/2, range=p2.x_range), top='PE_OI', source=source, color="#1E90FF", width=width,
            fill_alpha=0.5)
    ...
    p3 = figure(name='nb', y_range = (0, int(nifty_fut['Open Interest'].max())),plot_width=650, plot_height=350, y_minor_ticks=2, x_range=list(nifty_fut['Axis']),
    ...
    p3.extra_y_ranges = {"y2": Range1d(start=int(nifty_fut['Close'].min() - (nifty_fut['Close'].min()*0.02)), end=int(nifty_fut['Close'].max()+ (nifty_fut['Close'].max()*0.02)))}
    ...
    p4 = figure(name='mpc', x_range=(df['strikePrice'].iloc[0], df['strikePrice'].iloc[-1]),
                ...
                toolbar_location="above")
    p4v1 = p4.vbar(x=dodge('strikePrice', 0, range=p4.x_range), top='Total', source=source, color="#1E90FF", width=width,
            fill_alpha=0.5)
    ...
    data_table = DataTable(name = 'signals', source=sourceTable, columns=columns, width=600, height=280,  background="#0b0c11")
    ...
    data_table2 = DataTable(name = 'signals2', source=sourceTable2, columns=columns, width=600, height=280,  background="#0b0c11" )
